...
class GridFIT3D:
    ...
    stl_method: {'enclosed_points','interior_points','implicit_distance','voxelize_rectlinear'}, 
        default 'enclosed_points'
        Which algorithm to use when initially marking cells inside the STL
        solids.  This value is passed directly to ``_mark_cells_in_stl``.
    ...
    def __init__(self, xmin=None, xmax=None,
        ...
        # algorithm used to mark STL cells (default may be overridden later)
    ...
    def _mark_cells_in_stl(self,method):
        ...
        progress_bar = False
        #pacing=[(self.xmax - self.xmin) / self.Nx, (self.ymax - self.ymin) / self.Ny, (self.zmax - self.zmin) / self.Nz]
        spacing = [self.x[1] - self.x[0], self.y[1] - self.y[0], self.z[1] - self.z[0]]
        ...
        for key in self.stl_solids.keys():
            ...
            if method=='enclosed_points':
                ...
                #self.grid[key] = select.point_data_to_cell_data()['selected_points'] > stl_tolerance
                del select
            
            
            elif method == 'interior_points_signed':
                #point cloud of centers of cells in your grid
                cell_centers = self.grid.cell_centers()
                try:
                    select = cell_centers.select_interior_points(surf)
                except Exception:
                    select = cell_centers.select_interior_points(surf, check_surface=False)
                    if self.verbose > 1:
                        print(f'[!] Warning: stl solid {key} issues. Check STL closure.')

                # Direct mapping
                self.grid[key] = select['selected_points'].astype(bool)
                del select, cell_centers
                ...
                try:
                    ...
                    self.grid[key] = select.point_data_to_cell_data()['implicit_distance'] < 0
                except Exception:
            ...
            elif method=='implicit_distance_tol':
                # mark cells in stl [True == in stl, False == out stl]
                # negative distance is inside, positive is outside
                try:
                    select = self.grid.compute_implicit_distance(surf)
                    self.grid[key] = select.point_data_to_cell_data()['implicit_distance'] < 0.001
                except Exception:
                    print(f'[!] Warning: Implicit distance computation for stl solid {key} failed.')
                ...
                try:
                    ...
                    self.grid[key] = vox.cell_data['mask'].astype(bool)
                except Exception:
                    print(f'[!] Warning: voxelization for stl solid {key} failed. Consider checking if the grid is uniform, check the STL file or using a different method.')

                '''# mark cells in stl [True == in stl, False == out stl]
                # PyVista voxelization produces a UniformGrid with a cell_data
                # array called 'mask' (uint8) indicating inside/outside.  we need
                # to map that back onto our structured grid.  Two problems arise:
                # 1) the original ``__init__`` call already filled ``self.grid[key]``
                #    with the default enclosed_points mask, so if voxelization fails
                #    we must ensure we don't silently keep the old array (which
                #    was happening due to a spelling typo and the broad except
                #    catching the AttributeError).
                # 2) ``surf.voxelize_rectilinear`` may produce a different number of
                #    cells than the structured grid; we sample the grid cell
                #    centres to index into the voxelized mask.
                try:
                    vox = surf.voxelize_voxelize_rectilinear(spacing=spacing)

                    # convert the returned mask to boolean
                    if 'mask' in vox.cell_data:
                        vox_mask = vox.cell_data['mask'].astype(bool)
                    else:
                        # older versions used 'values'
                        vox_mask = vox.cell_data.get('values', np.array([])).astype(bool)

                    # if the masks align in size we can assign directly
                    if vox_mask.size == self.grid.n_cells:
                        self.grid[key] = vox_mask
                    else:
                        # otherwise sample the structured grid cell centres
                        centres = self.grid.cell_centers().points
                        # compute index in voxel grid for each centre
                        origin = np.array(vox.origin)
                        # spacing is (dx,dy,dz) as used above
                        idx = np.floor((centres - origin) / np.array(spacing)).astype(int)
                        # clip indices
                        max_idx = np.array(vox.dimensions) - 2  # cells = dims-1
                        idx = np.minimum(np.maximum(idx, 0), max_idx)
                        lin = np.ravel_multi_index(idx.T, max_idx + 1)
                        self.grid[key] = vox_mask[lin]

                except AttributeError as e:
                    # catch miss‑spelled method or missing feature
                    print(f"[!] Warning: could not voxelize STL (maybe wrong PyVistamethod?). {e}")
                except Exception as e:
                    # general failure (non‑uniform grid, etc.)
                    print(f"[!] Warning: voxelize_rectilinear failed: {e}")
                '''
                # note: previously we printed a misleading warning and left the
                # original enclosed_points mask in place; the corrected logic
                # above prevents that.
